...
import { MarcaApi } from "./services/endpoints";
...
const ModalProductEditar = ({close,id,name,refreshData}:{close:()=>void,id:number,name:string,refreshData:()=>void}) => {
  ...
  const handleSubmit = async ()=>{
    await MarcaApi.update({name:name_,id});
    refreshData();
    close();
  }
  return (
  ...
  return (
    <div>
      <div className="flex justify-center mb-4">
        <button onClick={()=>setOpenCreateModal(true)} className="px-4 py-2 rounded-md bg-slate-300">Agregar Marca</button>
